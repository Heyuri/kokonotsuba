...
// high level encapsulation of article (thread or post) queries
class PostData {	
	public $no;//postID
	...
	public $lastBumpTime;//last time thread has been bumped. root in db
	public $time;//prob post time but unix mili bc brain damage
	...
	// constructor for class, takes row id then assigns all properties
	function __construct($rowid, $PIO){
		$qry_values = $PIO->fetchPosts($rowid);
		// property assignment
		$this->no	= $qry_values[0]['no'];
		$this->resto	= $qry_values[0]['resto'];
		$this->category	= $qry_values[0]['category'];

		$this->lastBumpTime = $qry_values[0]['root'];
		$this->time	= $qry_values[0]['time'];
		$this->now	= $qry_values[0]['now'];
		
		$this->md5chksum = $qry_values[0]['md5chksum'];
		$this->fname	= $qry_values[0]['fname'];
		$this->ext	= $qry_values[0]['ext'];
		$this->imgw	= $qry_values[0]['imgw'];
		$this->imgh	= $qry_values[0]['imgh'];
		$this->imgsize	= $qry_values[0]['imgsize'];
		$this->tw	= $qry_values[0]['tw'];
		$this->th	= $qry_values[0]['th'];
		$this->tim	= $qry_values[0]['tim'];

		$this->pwd	= $qry_values[0]['pwd'];
		$this->name	= $qry_values[0]['name'];
		$this->email	= $qry_values[0]['email'];
		$this->sub	= $qry_values[0]['sub'];
		$this->com	= $qry_values[0]['com'];
		$this->host	= $qry_values[0]['host'];
		$this->status	= $qry_values[0]['status'];
	}
	/* Delete article */
	public function removePost($PIO){
		$post = $PIO->fetchPosts($this->no);
		$files = $PIO->removePosts(array($post[0]['no']));
	}
