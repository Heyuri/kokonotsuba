...
class PIOlogflockp implements IPIO {
	...
	function _AnalysisLogs($line){
		$tline = array();
		list($tline['no'], $tline['resto'], $tline['md5chksum'], $tline['category'], $tline['tim'], $tline['fname'], $tline['ext'], $tline['imgw'], $tline['imgh'], $tline['imgsize'], $tline['tw'], $tline['th'], $tline['pwd'], $tline['now'], $tline['name'], $tline['email'], $tline['sub'], $tline['com'], $tline['host'], $tline['status']) = explode(',', $line);
		return $tline; // list()是由右至左代入的
	}
	...
	function dbInit($isAddInitData = true){
		...
		foreach($chkfile as $value){
			if(!is_file($value)){ // 檔案不存在
				...
				if($value==$this->logfile) fwrite($fp, '1,0,,,0,,,0,0,,0,0,,,<b>'.$this->ENV['NONAME'].'</b>,,'.$this->ENV['NOTITLE'].',You find yourself in a mysterious place.,,,'); // PIO Structure V3
...
//		flock($fp, LOCK_UN); // 解鎖
	...
	/* thread ageru */
	function bumpThread($tno){
		if(!$this->prepared) $this->dbPrepare();
		if(!$this->isThread($tno)) return false;
		$torder_flip = array_flip($this->torder);
		if(isset($torder_flip[$tno])) unset($this->torder[$torder_flip[$tno]]);
		array_unshift($this->torder, $tno);
		return true;
	}

	/* 新增文章/討論串 */
	function addPost($no, $resto, $md5chksum, $category, $tim, $fname, $ext, $imgw, $imgh, $imgsize, $tw, $th, $pwd, $now, $name, $email, $sub, $com, $host, $age=false, $status='') {
		if(!$this->prepared) $this->dbPrepare();

		$tline = array($no, $resto, $md5chksum, $category, $tim, $fname, $ext, $imgw, $imgh, $imgsize, $tw, $th, $pwd, $now, $name, $email, $sub, $com, $host, $status);
		...
		if($age&&$resto) $this->bumpThread($resto?$resto:$no);
	}
		...
		for($i = 0; $i < $pcount; $i++){
			...
			if ($method=='REG') {
				foreach ($keyword as $k) {
					if (preg_match($k, $logsarray[0][$field])) array_push($foundPosts, $logsarray[0]);
				}
			} else {
				foreach($keyword as $k){
					if(strpos($logsarray[0][$field], $k)!==FALSE) $find++;
					if($method=="OR" && $find) break;
				}
				if($method=="AND" && $find==$keyword_cnt) array_push($foundPosts, $logsarray[0]); // 全部都有找到 (AND交集搜尋)
				elseif($method=="OR" && $find) array_push($foundPosts, $logsarray[0]); // 有找到 (OR聯集搜尋)
			}
		}
		return $foundPosts;
	}
