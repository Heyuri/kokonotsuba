...
class board {
	...
	public function rebuildBoard($resno = 0,$pagenum = -1,$single_page = false, $last = -1): void {
		$this->config = $this->loadBoardConfig();
		if(empty($this->config)) die("No board config for {$this->board_title}:{$this->board_uid}");
		$dbSettings = getDatabaseSettings();
		...
		if(!$resno){
			if($pagenum==-1){ // Rebuild mode (PHP dynamic output of multiple pages)
				...
				if($pagenum < 0 || ($pagenum * $this->config['PAGE_DEF']) >= $threads_count) $globalHTML->error( _T('page_not_found')); // $Pagenum is out of range
			...
			if(!$PIO->isThread($thread_uid)){ // Try to find the thread by child post no. instead
				...
				$redirectString = $this->config['PHP_SELF']."?res=".$resnoNew."#p".$this->getBoardUID()."_$resno";
			...
			if($tree_count && $this->config['RE_PAGE_DEF']){ // There is a response and RE_PAGE_DEF > 0 to do the pagination action
				if($pagenum==='all'){ // show all
					...
					if($pagenum * $this->config['RE_PAGE_DEF'] >= $tree_count) $globalHTML->error(_T('page_not_found'));
