	...
	public function onRenderPost(array &$arrLabels, array $post){
		...
		if(ip2long($iphost)!==false) {
			...
			if ($CountryName != "" || $record != ""){
				$arrLabels['{$NAME}'] .= ' <img class="countryFlag" src="' . $this->staticUrl . 'image/flag/'.strtolower($CountryID).'.png" title="'.$CountryName.'" alt="'.$CountryID.'">';
			} else {
			...
			if ($CountryName != "" || $record != ""){
				$arrLabels['{$NAME}'] .= ' <img src="' . $this->staticUrl . 'image/flag/'.strtolower($CountryID).'.png" title="'.$CountryName.'" style="vertical-align: middle;margin-top: -2px;" alt="'.$CountryID.'">';
			} else {
				$arrLabels['{$NAME}'] .= ' <img src="' . $this->staticUrl . 'image/flag/xx.png" title="Unknown" style="vertical-align: middle;margin-top: -2px;" alt="XX">';
			}
			}
	}
