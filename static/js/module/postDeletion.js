(function () {
	...
	function appendWarning(postEl, type) {
		...
		if (type === 'post' && !existsDeleted) {
			...
			infoExtra.appendChild(spacer);
			infoExtra.appendChild(warn);
			// Return appended nodes so they can be removed if needed
			return { warn: warn, spacer: spacer };
	...
	document.addEventListener('click', function (e) {
			...
			if (postEl.classList.contains('op')) {
				...
				if (thread) {
					...
					for (let i = 0; i < postsInThread.length; i++) {
						const res = appendWarning(postsInThread[i], 'post');
						if (res && res.spacer) appendedNodes.push(res.spacer);
		...
		function revertUI() {
			...
			// Remove nodes we appended (warnings + spacers)
