...
function onUserPostToBoard(){
	$postData = new postDataClass();
	$hookObj = HookClass::getInstance();
	global $conf;

	// make sure the client save there own password and name when they come back
	setrawcookie('namec', rawurlencode($_POST['name']), $conf->cookieExpireTime);
	setrawcookie('passwordc', rawurlencode($_POST['pwd']), $conf->cookieExpireTime);

	$hookObj->executeHook("onUserPostToBoard", $postData);// HOOK moduels can change things like maxFileSize, forceAllowFileType, CleanseComment
	
	$postData->loadDataFromPostRequest(); // loads data from post request
	$postData->cleansePost(); // remove any html tags stuff from the post
	$postData->procssesFiles(); // do things like make thumbnail. make hash. etc.

	//$hookObj->executeHook("sys_onPostFullyLoaded", $postData); // HOOK this is ment for internal moduels
	$hookObj->executeHook("onPostFullyLoaded", $postData); // HOOK the object is loaded. you might want to add stuff like bb code or emotes or tripcode.
