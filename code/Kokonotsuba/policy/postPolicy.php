...
class postPolicy extends policyBase {
	public function canDeletePost(string $passwordHash, string $suppliedPassword): bool {
		...
		if($this->canStaffDelete()) {
			return true;
		}

		else if(empty($passwordHash)) {
			// if there is no password hash for the post - then the post is not password protected, and nobody can delete it
			return false;
		}
