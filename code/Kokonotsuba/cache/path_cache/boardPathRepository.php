...
class boardPathRepository {
	...
	public function fetchById(int $id): ?cachedBoardPath {
		...
		return $this->databaseConnection->fetchAsClass($query, [$id], '\Kokonotsuba\cache\path_cache\cachedBoardPath') ?: null;
	}

	public function fetchByBoardUid(int $uid): ?cachedBoardPath {
		$query = "SELECT * FROM {$this->boardPathTable} WHERE boardUID = ?";
		return $this->databaseConnection->fetchAsClass($query, [$uid], '\Kokonotsuba\cache\path_cache\cachedBoardPath') ?: null;
	}

	public function fetchAll(): array {
		$query = "SELECT * FROM {$this->boardPathTable}";
		return $this->databaseConnection->fetchAllAsClass($query, [], '\Kokonotsuba\cache\path_cache\cachedBoardPath');
	}
