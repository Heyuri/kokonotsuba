...
/** The links drawn on a post: category search links and the quote, reply and recent-replies buttons. */
final class postElementGenerator {
	 ...
	 *
	 * @param string     $category  Category field as stored.
	 * @param string     $crossLink Board prefix for the search URL.
	 * @param textFormat $format    The post's stored text format; plain-text categories are escaped here.
	 */
	public function processCategoryLinks(string $category, string $crossLink, textFormat $format = textFormat::LEGACY_HTML): string {
		if (!$this->board->getConfigValue('USE_CATEGORY')) return '';
		$categories = explode(',', str_replace('&#44;', ',', $category));
		$categories = array_map('trim', $categories);
		$escape = !$format->fieldsAreHtml();
		return implode(', ', array_map(function ($c) use ($crossLink, $escape) {
			$label = $escape ? sanitizeStr($c) : $c;
			return '<a href="'.$crossLink.$this->board->getConfigValue('LIVE_INDEX_FILE') . '?mode=module&load=mod_searchcategory&c='.urlencode($c).'">'.$label.'</a>';
