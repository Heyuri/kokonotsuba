	...
	private function renderSinglePost(
		...
		$templateValues += $isReply
			? bindReplyValuesToTemplate(
				$this->board, $this->config, $data['post_uid'], $data['no'], $postOPNumber, $data['sub'], $nameHtml, $data['now'],
				$categoryHTML, $QUOTEBTN, $IMG_BAR, $data['imgsrc'] ?? '', $WARN_BEKILL, $data['com'], $POSTFORM_EXTRA, '', $BACKLINKS, $thread_uid
			)
			: bindOPValuesToTemplate(
				$this->board, $this->config, $data['post_uid'], $data['no'], $data['sub'], $nameHtml, $data['now'],
	...
	private function preparePostData(array $post): array {
		...
		if ($ext) {
			...
			if ($tw && $th && $thumbName) {
				...
				$imgsrc = '<a href="'.$imageURL.'" target="_blank" rel="nofollow"><img src="'.$this->config['STATIC_URL'].'image/nothumb.gif" class="postimg" alt="'.$imgsize.'"></a>'; // Default display style 	(when no preview image)
			}
		}
