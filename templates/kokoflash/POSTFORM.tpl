		...
		<form id="postform" name="postform" action="{$LIVE_INDEX_FILE}" method="POST" <!--&IF($MAX_FILE_SIZE,' enctype="multipart/form-data"','')--> {$ALWAYS_NOKO}>
			...
			<table id="postformTable">
				<tbody>
					<tr>
						<td class="postblock"><label for="name">{$FORM_NAME_LABEL}</label></td>
						<td class="postformInputCell">{$FORM_NAME_FIELD}</td>
					</tr>
					<tr>
						<td class="postblock"><label for="email">{$FORM_EMAIL_LABEL}</label></td>
						<td class="postformInputCell">{$FORM_EMAIL_FIELD}<!--&NOKO_SAGE_DUMP/--></td>
					</tr>
					<tr>
						<td class="postblock">
							<label for="sub">{$FORM_TOPIC_LABEL}</label></td>
						<td class="postformInputCell">{$FORM_TOPIC_FIELD}{$FORM_SUBMIT}</td>
					</tr>
					<tr>
						<td class="postblock">
							<label for="com">{$FORM_COMMENT_LABEL}</label>{$FORM_COMMENT_BLOCK_EXTRA}</td>
						<td class="postformInputCell">{$FORM_COMMENT_FIELD}{$FORM_COMMENT_EXTRAS}</td>
					</tr>
					<!--&IF($FORM_ATTECHMENT_FIELD,'<tr>
						<td class="postblock"><label for="upfile">{$FORM_FILE_LABEL}</label></td>
						<td class="postformInputCell">{$FORM_ATTECHMENT_FIELD}
							<div id="postformFileOptionsContainer">','')-->
								<!--&IF($FORM_CONTPOST_FIELD,'<div id="continuousContainer"><label id="continuousLabel">{$FORM_CONTPOST_FIELD}{$FORM_CONTPOST_LABEL}</label></div>','')-->
					...
					<!--&IF($FORM_CATEGORY_FIELD,'<tr>
						<td class="postblock"><label for="category">{$FORM_CATEGORY_LABEL}</label></td>
						<td class="postformInputCell">{$FORM_CATEGORY_FIELD}<small></small></td>
					</tr>','')-->
					<!--&IF($FORM_TAG_FIELD,'<tr>
						<td class="postblock"><label for="tag">{$FORM_TAG_LABEL}</label></td>
						<td class="postformInputCell">{$FORM_TAG_FIELD}</td>
					</tr>','')-->
					<tr>
						<td class="postblock"><label for="pwd">{$FORM_PASSWORD_LABEL}</label></td>
						<td class="postformInputCell"><input type="password" name="pwd" id="pwd" value="" class="inputtext" maxlength="{$INPUT_MAX}"><span id="delPasswordInfo">{$FORM_PASSWORD_NOTICE}</span>{$FORM_EXTRA_COLUMN}</td>
					</tr>
					<tr>
